...
func (m *Vpc) ToORM(ctx context.Context) (VpcORM, error) {
	to := VpcORM{}
	var err error
	if prehook, ok := interface{}(m).(VpcWithBeforeToORM); ok {
		if err = prehook.BeforeToORM(ctx, &to); err != nil {
			return to, err
		}
	}
	if v, err := resource1.DecodeInt64(&Vpc{}, m.Id); err != nil {
		return to, err
	} else {
		to.Id = v
	}
	...
	to.AccountID = accountID
	if posthook, ok := interface{}(m).(VpcWithAfterToORM); ok {
		err = posthook.AfterToORM(ctx, &to)
	}
	return to, err
}
...
func (m *VpcORM) ToPB(ctx context.Context) (Vpc, error) {
	to := Vpc{}
	var err error
	if prehook, ok := interface{}(m).(VpcWithBeforeToPB); ok {
		if err = prehook.BeforeToPB(ctx, &to); err != nil {
			return to, err
		}
	}
	if v, err := resource1.Encode(&Vpc{}, m.Id); err != nil {
		return to, err
	} else {
		to.Id = v
	}
	to.Name = m.Name
	to.Description = m.Description
	if posthook, ok := interface{}(m).(VpcWithAfterToPB); ok {
		err = posthook.AfterToPB(ctx, &to)
	}
	return to, err
}
...
// The following are interfaces you can implement for special behavior during ORM/PB conversions
// of type Vpc the arg will be the target, the caller the one being converted from
...
func DefaultCreateVpc(ctx context.Context, in *Vpc, db *gorm1.DB) (*Vpc, error) {
	...
	if hook, ok := interface{}(&ormObj).(VpcORMWithBeforeCreate_); ok {
		if db, err = hook.BeforeCreate_(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(VpcORMWithAfterCreate_); ok {
		if err = hook.AfterCreate_(ctx, db); err != nil {
			return nil, err
		}
	}
...
func DefaultReadVpc(ctx context.Context, in *Vpc, db *gorm1.DB, fs *query1.FieldSelection) (*Vpc, error) {
	...
	if hook, ok := interface{}(&ormObj).(VpcORMWithBeforeReadApplyQuery); ok {
		if db, err = hook.BeforeReadApplyQuery(ctx, db, fs); err != nil {
			return nil, err
		}
	}
	if db, err = gorm2.ApplyFieldSelection(ctx, db, fs, &VpcORM{}); err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(VpcORMWithBeforeReadFind); ok {
		if db, err = hook.BeforeReadFind(ctx, db, fs); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormResponse).(VpcORMWithAfterReadFind); ok {
		if err = hook.AfterReadFind(ctx, db, fs); err != nil {
			return nil, err
		}
	}
...
func DefaultDeleteVpc(ctx context.Context, in *Vpc, db *gorm1.DB) error {
	...
	if hook, ok := interface{}(&ormObj).(VpcORMWithBeforeDelete_); ok {
		if db, err = hook.BeforeDelete_(ctx, db); err != nil {
			return err
		}
	}
	err = db.Where(&ormObj).Delete(&VpcORM{}).Error
	if err != nil {
		return err
	}
	if hook, ok := interface{}(&ormObj).(VpcORMWithAfterDelete_); ok {
		err = hook.AfterDelete_(ctx, db)
	}
	return err
}
...
func DefaultDeleteVpcSet(ctx context.Context, in []*Vpc, db *gorm1.DB) error {
	...
	if hook, ok := (interface{}(&VpcORM{})).(VpcORMWithBeforeDeleteSet); ok {
		if db, err = hook.BeforeDeleteSet(ctx, in, db); err != nil {
			return err
		}
	}
	...
	err = db.Where("account_id = ? AND id in (?)", acctId, keys).Delete(&VpcORM{}).Error
	if err != nil {
		return err
	}
	if hook, ok := (interface{}(&VpcORM{})).(VpcORMWithAfterDeleteSet); ok {
		err = hook.AfterDeleteSet(ctx, in, db)
	}
	return err
}
...
func DefaultStrictUpdateVpc(ctx context.Context, in *Vpc, db *gorm1.DB) (*Vpc, error) {
	if in == nil {
		return nil, fmt.Errorf("Nil argument to DefaultStrictUpdateVpc")
	}
	ormObj, err := in.ToORM(ctx)
	if err != nil {
		return nil, err
	...
	lockedRow := &VpcORM{}
	db.Model(&ormObj).Set("gorm:query_option", "FOR UPDATE").Where("id=?", ormObj.Id).First(lockedRow)
	if hook, ok := interface{}(&ormObj).(VpcORMWithBeforeStrictUpdateCleanup); ok {
		if db, err = hook.BeforeStrictUpdateCleanup(ctx, db); err != nil {
			return nil, err
		}
	}
	if hook, ok := interface{}(&ormObj).(VpcORMWithBeforeStrictUpdateSave); ok {
		if db, err = hook.BeforeStrictUpdateSave(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(VpcORMWithAfterStrictUpdateSave); ok {
		if err = hook.AfterStrictUpdateSave(ctx, db); err != nil {
			return nil, err
		}
	}
...
func DefaultPatchVpc(ctx context.Context, in *Vpc, updateMask *field_mask1.FieldMask, db *gorm1.DB) (*Vpc, error) {
	...
	var pbObj Vpc
	var err error
	if hook, ok := interface{}(&pbObj).(VpcWithBeforePatchRead); ok {
		if db, err = hook.BeforePatchRead(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	pbReadRes, err := DefaultReadVpc(ctx, &Vpc{Id: in.GetId()}, db, nil)
	if err != nil {
		return nil, err
	}
	pbObj = *pbReadRes
	if hook, ok := interface{}(&pbObj).(VpcWithBeforePatchApplyFieldMask); ok {
		if db, err = hook.BeforePatchApplyFieldMask(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	if _, err := DefaultApplyFieldMaskVpc(ctx, &pbObj, in, updateMask, "", db); err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&pbObj).(VpcWithBeforePatchSave); ok {
		if db, err = hook.BeforePatchSave(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	pbResponse, err := DefaultStrictUpdateVpc(ctx, &pbObj, db)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(pbResponse).(VpcWithAfterPatchSave); ok {
		if err = hook.AfterPatchSave(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
...
func DefaultPatchSetVpc(ctx context.Context, objects []*Vpc, updateMasks []*field_mask1.FieldMask, db *gorm1.DB) ([]*Vpc, error) {
	...
	results := make([]*Vpc, 0, len(objects))
	for i, patcher := range objects {
		pbResponse, err := DefaultPatchVpc(ctx, patcher, updateMasks[i], db)
		if err != nil {
			return nil, err
		}
...
func DefaultListVpc(ctx context.Context, db *gorm1.DB, f *query1.Filtering, s *query1.Sorting, p *query1.Pagination, fs *query1.FieldSelection) ([]*Vpc, error) {
	...
	if hook, ok := interface{}(&ormObj).(VpcORMWithBeforeListApplyQuery); ok {
		if db, err = hook.BeforeListApplyQuery(ctx, db, f, s, p, fs); err != nil {
			return nil, err
		}
	}
	db, err = gorm2.ApplyCollectionOperators(ctx, db, &VpcORM{}, &Vpc{}, f, s, p, fs)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(VpcORMWithBeforeListFind); ok {
		if db, err = hook.BeforeListFind(ctx, db, f, s, p, fs); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(VpcORMWithAfterListFind); ok {
		if err = hook.AfterListFind(ctx, db, &ormResponse, f, s, p, fs); err != nil {
			return nil, err
		}
	}
...
func (m *VpcsDefaultServer) Create(ctx context.Context, in *CreateVpcRequest) (*CreateVpcResponse, error) {
	...
	if custom, ok := interface{}(in).(VpcsVpcWithBeforeCreate); ok {
		var err error
		if db, err = custom.BeforeCreate(ctx, db); err != nil {
			return nil, err
		}
	}
	res, err := DefaultCreateVpc(ctx, in.GetPayload(), db)
	if err != nil {
		return nil, err
	}
	out := &CreateVpcResponse{Result: res}
	if custom, ok := interface{}(in).(VpcsVpcWithAfterCreate); ok {
		var err error
		if err = custom.AfterCreate(ctx, out, db); err != nil {
			return nil, err
		}
...
func (m *VpcsDefaultServer) Read(ctx context.Context, in *ReadVpcRequest) (*ReadVpcResponse, error) {
	...
	if custom, ok := interface{}(in).(VpcsVpcWithBeforeRead); ok {
		var err error
		if db, err = custom.BeforeRead(ctx, db); err != nil {
			return nil, err
		}
	}
	res, err := DefaultReadVpc(ctx, &Vpc{Id: in.GetId()}, db, in.Fields)
	if err != nil {
		return nil, err
	}
	out := &ReadVpcResponse{Result: res}
	if custom, ok := interface{}(in).(VpcsVpcWithAfterRead); ok {
		var err error
		if err = custom.AfterRead(ctx, out, db); err != nil {
			return nil, err
		}
...
func (m *VpcsDefaultServer) Update(ctx context.Context, in *UpdateVpcRequest) (*UpdateVpcResponse, error) {
	...
	if custom, ok := interface{}(in).(VpcsVpcWithBeforeUpdate); ok {
		var err error
		if db, err = custom.BeforeUpdate(ctx, db); err != nil {
			return nil, err
		}
	...
	if in.GetFields() == nil {
		res, err = DefaultStrictUpdateVpc(ctx, in.GetPayload(), db)
	} else {
		res, err = DefaultPatchVpc(ctx, in.GetPayload(), in.GetFields(), db)
	}
	if err != nil {
		return nil, err
	}
	out := &UpdateVpcResponse{Result: res}
	if custom, ok := interface{}(in).(VpcsVpcWithAfterUpdate); ok {
		var err error
		if err = custom.AfterUpdate(ctx, out, db); err != nil {
			return nil, err
		}
...
func (m *VpcsDefaultServer) Delete(ctx context.Context, in *DeleteVpcRequest) (*DeleteVpcResponse, error) {
	...
	if custom, ok := interface{}(in).(VpcsVpcWithBeforeDelete); ok {
		var err error
		if db, err = custom.BeforeDelete(ctx, db); err != nil {
			return nil, err
		}
	}
	err := DefaultDeleteVpc(ctx, &Vpc{Id: in.GetId()}, db)
	if err != nil {
		return nil, err
	}
	out := &DeleteVpcResponse{}
	if custom, ok := interface{}(in).(VpcsVpcWithAfterDelete); ok {
		var err error
		if err = custom.AfterDelete(ctx, out, db); err != nil {
			return nil, err
		}
...
func (m *VpcsDefaultServer) List(ctx context.Context, in *ListVpcRequest) (*ListVpcsResponse, error) {
	...
	if custom, ok := interface{}(in).(VpcsVpcWithBeforeList); ok {
		var err error
		if db, err = custom.BeforeList(ctx, db); err != nil {
			return nil, err
		}
	...
	res, err := DefaultListVpc(ctx, db, in.Filter, in.OrderBy, in.Paging, in.Fields)
	if err != nil {
		return nil, err
	}
	var resPaging *query1.PageInfo
	...
	out := &ListVpcsResponse{Results: res, Page: resPaging}
	if custom, ok := interface{}(in).(VpcsVpcWithAfterList); ok {
		var err error
		if err = custom.AfterList(ctx, out, db); err != nil {
			return nil, err
		}
